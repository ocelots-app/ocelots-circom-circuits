...
describe("start testing disclose.circom", function () {
  ...
  before(async () => {
    circuit = await wasm_tester(
      path.join(__dirname, "../circuits/disclose.circom"),
      {
        include: [
          "node_modules",
    ...
    const secret = BigInt(
      Math.floor(Math.random() * Math.pow(2, 254))
    ).toString();
  ...
  it("should have nullifier == poseidon(secret, scope)", async function () {
    ...
    const nullifier_circom = (await circuit.getOutput(w, ["nullifier"]))
      .nullifier;
  ...
  describe("MRZ selective disclosure", function () {
    ...
    attributeCombinations.forEach((combination) => {
      it(`Disclosing ${combination.join(", ")}`, async function () {
        const attributeToReveal = Object.keys(attributeToPosition).reduce(
          (acc, attribute) => {
        ...
        const revealedData_packed = await circuit.getOutput(w, [
          "revealedData_packed[3]",
        ]);
        ...
        for (let i = 0; i < reveal_unpacked.length; i++) {
          if (bitmap[i] == "1") {
            ...
            assert(
              reveal_unpacked[i] == char,
              "Should reveal the right character"
            );
          } else {
