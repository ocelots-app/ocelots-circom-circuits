...
describe("Proof of Passport - Circuits - Register flow", function () {
  ...
  before(async () => {
    circuit = await wasm_tester(
      path.join(
        __dirname,
        "../circuits/register_sha256WithRSAEncryption_65537.circom"
      ),
      {
        include: [
          "node_modules",
  ...
  it("should calculate the witness with correct inputs", async function () {
    ...
    console.log(
      "nullifier",
      (await circuit.getOutput(w, ["nullifier"])).nullifier
    );

    const commitment_circom = (await circuit.getOutput(w, ["commitment"]))
      .commitment;
  ...
  it("should fail to calculate witness with invalid merkle root", async function () {
    try {
      const invalidInputs = {
        ...inputs,
        merkle_root: inputs.merkle_root.map((byte: string) =>
          String((parseInt(byte, 10) + 1) % 256)
        ),
      };
      await circuit.calculateWitness(invalidInputs);
