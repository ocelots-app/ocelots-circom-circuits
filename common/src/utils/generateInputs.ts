...
export function generateCircuitInputsRegister(
  ...
  if (options.developmentMode) {
    tree.insert(
      getLeaf({
        signatureAlgorithm:
          mockPassportData_sha256WithRSAEncryption_65537.signatureAlgorithm,
        issuer:
          "C = TS, O = Government of Syldavia, OU = Ministry of tests, CN = CSCA-TEST",
        modulus: mockPassportData_sha256WithRSAEncryption_65537.pubKey.modulus,
        exponent:
          mockPassportData_sha256WithRSAEncryption_65537.pubKey.exponent,
      }).toString()
    );
  }

  if (
    !["sha256WithRSAEncryption", "sha1WithRSAEncryption"].includes(
      passportData.signatureAlgorithm
    )
  ) {
    console.error(
      `${passportData.signatureAlgorithm} not supported for proof right now.`
    );
    throw new Error(
      `${passportData.signatureAlgorithm} not supported for proof right now.`
    );
  ...
  const concatenatedDataHashes = passportData.dataGroupHashes;
  console.log(passportData);
  const concatenatedDataHashesHashDigest = hash(
    passportData.signatureAlgorithm,
  ...
  assert(
    arraysAreEqual(
      passportData.eContent.slice(
        72,
        72 + getDigestLengthBytes(passportData.signatureAlgorithm)
      ),
      concatenatedDataHashesHashDigest
    ),
  ...
  if (passportData.dataGroupHashes.length > MAX_DATAHASHES_LEN) {
    console.error(
      `Data hashes too long. Max length is ${MAX_DATAHASHES_LEN} bytes.`
    );
    throw new Error(
      `This number of datagroups is currently unsupported. Please contact us so we add support!`
    );
  }

  const [messagePadded, messagePaddedLen] = shaPad(
    passportData.signatureAlgorithm,
    new Uint8Array(concatenatedDataHashes),
    MAX_DATAHASHES_LEN
  );
  ...
  return {
    ...
    signed_attributes: passportData.eContent
      .map(toUnsignedByte)
...
export function generateCircuitInputsDisclose(
  ...
  const { merkleProofSiblings, merkleProofIndices, depthForThisOne } =
    generateMerkleProof(merkletree, index, PUBKEY_TREE_DEPTH);
  ...
  return {
    ...
    bitmap: bitmap,
    scope: [scope],
    current_date: getCurrentDateYYMMDD().map((datePart) =>
      BigInt(datePart).toString()
    ),
